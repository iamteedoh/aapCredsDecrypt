...
#
# Non-interactive (quiet) mode processing.
#
def run_non_interactive(args):
    # --list-cred-types
    if args.list_cred_types:
        types = list_used_credential_types()
        if not types:
            print("No credential types found.\n")
        else:
            print("Used Credential Types:")
            for ct in types:
                print(f"  {ct.id}) {ct.name}")
    # --export: Export ALL credentials
    if args.export:
        if not args.export_file:
            print("Error: --export requires --export-file to be provided.")
            sys.exit(1)
        print("Decrypting ALL credentials for export...\n")
        decrypted = decrypt_all_credentials()
        for cred in decrypted:
            unique_jts = {tuple(sorted(d.items())) for d in cred['related_job_templates']}
            cred['related_job_templates'] = [dict(t) for t in unique_jts]
        ...
        try:
            with open(args.export_file, "w") as f:
                f.write(output_json)
            print(f"Credentials exported to {args.export_file}\n")
        except Exception as e:
            print(f"Error writing export file: {e}")
        # --encrypt: Encrypt the exported file using ansible-vault if requested.
        if args.encrypt:
            if not args.vault_password_file:
                print("Error: --encrypt requires --vault-password-file to be provided.")
                sys.exit(1)
            try:
                subprocess.check_call([
                    "ansible-vault", "encrypt", args.export_file,
                    "--vault-password-file", args.vault_password_file
                ])
                print(f"Export file {args.export_file} encrypted using ansible-vault.\n")
            except Exception as e:
    ...
    if args.import_flag:
        if not args.import_file:
            print("Error: --import requires --import-file to be provided.")
            sys.exit(1)
        # Check if the file appears to be ansible-vault encrypted.
        try:
            with open(args.import_file, "r") as f:
                first_line = f.readline()
            if first_line.startswith("$ANSIBLE_VAULT;"):
                if not args.vault_password_file:
                    print("Error: The import file appears to be encrypted but no --vault-password-file provided.")
                    sys.exit(1)
                ...
                try:
                    subprocess.check_call([
                        "ansible-vault", "decrypt", args.import_file,
                        "--vault-password-file", args.vault_password_file,
                        "--output", temp_path
                    ])
                    print(f"Decrypted import file to temporary file: {temp_path}")
                    import_credentials_from_file(temp_path)
                finally:
                    os.remove(temp_path)
            else:
                import_credentials_from_file(args.import_file)
        except Exception as e:
            print(f"Error processing import file: {e}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="AWX/AAP Credential Export/Import Tool")
    parser.add_argument("--quiet", action="store_true",
                        help="Run in non-interactive mode")
    parser.add_argument("--export", action="store_true",
                        help="Export credentials")
    parser.add_argument("--export-file", type=str,
                        help="File path to export credentials to (must be .json)")
    parser.add_argument("--import", dest="import_flag", action="store_true",
                        help="Import credentials")
    parser.add_argument("--import-file", type=str,
                        help="File path to import credentials from (must be .json)")
    parser.add_argument("--list-cred-types", action="store_true",
                        help="List used credential types")
    parser.add_argument("--encrypt", action="store_true",
                        help="Encrypt the exported file using ansible-vault")
    parser.add_argument("--vault-password-file", type=str,
                        help="Full path of the vault password file for vault operations")
