...
## Copyright (C) 2025 TEEDOH LABS INC copying@teedohlabs.com
##
## This file is part of aapCredsDecrypt.
##
## aapCredsDecrypt is free software: you can redistribute it and/or modify
## it under the terms of the GNU General Public License as published by
## the Free Software Foundation, either version 3 of the License, or
## (at your option) any later version.
##
## aapCredsDecrypt is distributed in the hope that it will be useful,
...
def interactive_menu():
    """
    The main interactive loop providing a menu for the user.
    """
    while True:
        print("-------------------------------------------------")
        print("Main Menu:")
...
class Command(BaseCommand):
    help = "Manage AWX/AAP credentials export/import interactively or non-interactively"

    def add_arguments(self, parser):
        parser.add_argument('--export', action='store_true', help='Export credentials')
        parser.add_argument('--import', dest='import_', action='store_true', help='Import credentials')
    ...
    def handle(self, *args, **options):
        ...
        if quiet:
            if export:
                ...
                self.stdout.write("Running non-interactive import...")
                if not import_file:
                    self.stderr.write("Error: --import-file must be specified for import.")
                    return
                import_credentials_from_file(import_file)
            else:
                self.stdout.write("Quiet mode requires either --export or --import.")
        else:
            # Run interactive mode
            self.stdout.write("Running interactive mode...\n")
            interactive_menu()
